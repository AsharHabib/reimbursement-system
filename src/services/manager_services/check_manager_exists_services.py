...
from src.dao.manager_dao import check_manager_exists_dao
# Import info-level and warning-level loggers from logger.py
from src.logger import info_logger, my_logger

# Gets the manager usernames and passwords from the DAO, compares the indices for username and password in the
...
def check_manager_exists(username, password, productionDB=True):
    usernames, passwords = check_manager_exists_dao.check_manager_exists(productionDB)
    try:
